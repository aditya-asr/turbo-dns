...
tmux new-session -d -s 'docker-bind' -n 'bind' 'docker compose up ns1_example'
sleep 5
tmux split-window -t 'docker-bind:0' -h 'docker compose up ns1_root'
sleep 5
tmux split-window -t 'docker-bind:0' -v 'docker compose up resolver client1'
sleep 5
tmux split-window -t 'docker-bind:0.0' -v 'docker exec -it build-client1-1 /bin/bash'
sleep 5
tmux send-keys -t docker-bind:0.1 'dig +tries=1 +timeout=5 @172.20.0.2 test.example' Enter
sleep 5
